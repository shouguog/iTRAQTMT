...
public class Regression{
	...
	protected double sumSquares(Object regFun, double[] x){
	    ...
	    for(int i=0; i<this.nTerms; i++)param[i]=x[i]/this.scale[i];
	...
	protected void linearPrint(FileOutput fout){
        ...
        if(this.bestPolyFlag){
            ...
            fout.println("F-probability significance level (%): " + this.fProbSignificance*100.0);
            ...
            System.out.println((Integer)this.bestPolyArray.get(0) + "  " + nAttempts);
            ...
            for(int i=0; i<nAttempts; i++){
                ...
                fout.println(Fmath.truncate(fProbs[i], this.prec));
            }
    ...
    public double testOfAdditionalTermsFprobability(double chiSquareR, int nParametersR, double chiSquareF, int nParametersF, int nPoints){
        int degFreedomR = nPoints - nParametersR;
        int degFreedomF = nPoints - nParametersF;
    ...
    public double testOfAdditionalTermsFprobabilty(double chiSquareR, int nParametersR, double chiSquareF, int nParametersF, int nPoints){
        return testOfAdditionalTermsFprobability(chiSquareR, nParametersR, chiSquareF, nParametersF, nPoints);
    }
	...
	public static double[] fitCauchy(double[] array){
        ...
        System.out.println("Fitting a set of data to one or more distributions");
        System.out.println("Class Regression/Stat: method fitAllDistributions");
        ...
        System.out.print("Number of bins: ");
        System.out.println(numberOfBins);
        System.out.print("Bin width: ");
        System.out.println(binWidth);
        System.out.print("Average number of points per bin: ");
        System.out.println(averagePointsPerBin);
        System.out.println();
        //Shouguocase 5: // Lorentzian
        Regression reg = null;
        double[] coeff = null;
        reg = new Regression(array, binWidth);
        reg.supressPrint();
        //Shouguo we do not want to plot reg.lorentzianPlot();
        reg.lorentzian();
        coeff = reg.getCoeff();
        System.out.println("LORENTZIAN DISTRIBUTION");
        System.out.println("Best Estimates:");
        System.out.print("Mean [mu] ");
        System.out.println(coeff[0]);
        System.out.print("Half-height parameter [Gamma] ");
        System.out.println(coeff[1]);
        System.out.print("Scaling factor [Ao] ");
        System.out.println(coeff[2]);
        //Regression.regressionDetails(fout, reg);
        return coeff;//mu gamma scale
        }
	...
	public static void fitOneOrSeveralDistributions(double[] array){
        ...
        String outputTitle = "fitOneOrSeveralDistributionsOutput.txt";//Shouguo
        ...
        double[] coeff = null;
        while(testDistType){
                //Shouguo int opt =  Db.optionBox(headerComment, comments, boxTitles, defaultBox);
                int opt = 5;
                switch(opt){
                    case 1: // Gaussian
                            reg = new Regression(array, binWidth);
    ...
    protected void fitsexponentialMultiple(int nExps, int plotFlag){
        ...
        for(int i=0; i<this.nTerms; i+=2){
            // initial estimates
            start[i] = Math.exp(coeff[0])/this.nTerms;
            start[i+1] = coeff[1];
